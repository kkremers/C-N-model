...
LAI_avg = tapply(out_GS$LAI, out_GS$Tavg, mean)
TFN_avg = tapply(out_GS$TFN, out_GS$Tavg, mean)

LAI_max = tapply(out_GS$LAI, out_GS$Tavg, max)
TFN_max = tapply(out_GS$TFN, out_GS$Tavg, max)


par(mfrow=c(2,1))
plot(LAI_avg~TFN_avg, pch=16)
plot(LAI_max~TFN_max, pch=16)


par(mfrow=c(1,1))
plot(out$LAI~out$TFN)
