...
head(data.assim)
data.compare1 = data.assim[,c(1:3,7:10)] #pull out columns for data that you want to assimilate
...
for (i in 2:M) {
  ...
  out.compare1 = out[match(data.compare1$time, out$time),c(1:3,9,11,12,13)] #these columns need to match the ones that were pulled out before
...
save.image(file="Step1_NEE_GPP_Re_NDVI_BiomassCN.Rdata")
